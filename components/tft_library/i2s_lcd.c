...
static const char* I2S_TAG = "I2S_LCD";
...
static esp_err_t i2s_set_parallel_mode(i2s_port_t i2s_num)
{
	...
	// if((*(dma->buf) = (char *)malloc(sizeof(uint32_t) * DMA_SIZE)) == NULL) {
	//	   ESP_LOGE(I2S_TAG, "malloc dma buf fail");
	//	   goto _err;
	// }
	//memset(*(dma->buf), 0, sizeof(uint32_t) * DMA_SIZE);
...
// https://github.com/Makuna/NeoPixelBus/issues/379
...
esp_err_t i2s_lcd_pin_config(i2s_port_t i2s_num, i2s_paral_pin_config_t* pin_conf)
{
...
#elif defined CONFIG_IDF_TARGET_ESP32S2
	data_idx = I2S0O_DATA_OUT8_IDX;
#endif

	for(int i = 0; i < pin_conf->data_width; i++) {
		gpio_matrix_out(pin_conf->data_io_num[i], data_idx+i, false, false);
		//gpio_set_drive_capability(pin_conf->data_io_num[i], 3);
	}
	...
	//gpio_set_drive_capability(pin_conf->ws_io_num, 3);
	return ESP_OK;
}
